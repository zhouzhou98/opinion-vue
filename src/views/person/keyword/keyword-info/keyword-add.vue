<template>
  <div>
    <el-form ref="form" :model="keyword"  label-width="160px" size="medium">
      ...
      <el-form-item>
        <el-button type="primary"  icon="el-icon-s-promotion" @click="add(keyword)">提交</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      add(keyword){
        this.req.kname=keyword.kname
        this.req.description=keyword.description
        this.req.uid=JSON.parse(jsCookie.get('user')).id
        this.req.isOpen=keyword.isOpen
        
        KeywordApi.add(this.req).then(res=>{
